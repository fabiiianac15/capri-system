...
interface UpdateMontaDTO {
  ...
  criasHembra?: number;
  criasMacho?: number;
  detallesCrias?: string; // JSON con info de cada cría
...
class MontaService {
   ...
   */
  async create(data: CreateMontaDTO) {
  ...
  async getAll(filters?: {
    ...
    if (filters?.hembraId) where.hembraId = filters.hembraId;
    if (filters?.machoId) where.machoId = filters.machoId;
   ...
   * Actualizar monta
   */
  async update(id: string, data: UpdateMontaDTO) {
    const monta = await prisma.monta.findUnique({
      where: { id }
    });
   ...
   * Registrar parto
   */
  async registrarParto(data: RegistrarPartoDTO) {
    const monta = await prisma.monta.findUnique({
   ...
   * Registrar aborto
   */
  async registrarAborto(montaId: string, data: {
    fechaAborto: Date;
    motivo?: string;
    observaciones?: string;
  }) {
  ...
  async getGestacionesActivas() {
    ...
    const hoy = new Date();
    const gestacionesConDias = montas.map((monta: any) => {
   ...
   */
  async getProximosPartos(dias: number = 30) {
    const hoy = new Date();
   ...
   */
  async getHistorialHembra(hembraId: string) {
    const montas = await prisma.monta.findMany({
      where: {
        hembraId,
      },
    ...
    const partos = montas.filter((m: any) => m.tipoEvento === 'PARTO_EXITOSO');
    const totalCrias = partos.reduce((sum: number, m: any) => sum + (m.totalCrias || 0), 0);
    const abortos = montas.filter((m: any) => m.tipoEvento === 'ABORTO').length;
    ...
    return {
      ...
      estadisticas: {
        ...
        totalCrias,
        totalHembras: partos.reduce((sum: number, m: any) => sum + (m.criasHembra || 0), 0),
        totalMachos: partos.reduce((sum: number, m: any) => sum + (m.criasMacho || 0), 0),
        abortos,
   ...
   */
  async getHistorialMacho(machoId: string) {
    const montas = await prisma.monta.findMany({
      where: {
        machoId,
      },
  ...
  async getEstadisticas(filters?: {
    ...
    if (filters?.startDate || filters?.endDate) {
      where.fechaMonta = {};
      if (filters.startDate) where.fechaMonta.gte = new Date(filters.startDate);
      if (filters.endDate) where.fechaMonta.lte = new Date(filters.endDate);
    }

    const montas = await prisma.monta.findMany({ where });

    const totalMontas = montas.length;
    const gestacionesActivas = montas.filter((m: any) => m.tipoEvento === 'GESTACION').length;
    const partos = montas.filter((m: any) => m.tipoEvento === 'PARTO_EXITOSO');
    const abortos = montas.filter((m: any) => m.tipoEvento === 'ABORTO').length;
    const totalCrias = partos.reduce((sum: number, m: any) => sum + (m.totalCrias || 0), 0);

    // Machos más usados
    const montasPorMacho: { [key: string]: any } = {};
    montas.forEach((m: any) => {
    ...
    const topMachos = await Promise.all(
      Object.entries(montasPorMacho)
    ...
    return {
      ...
      abortos,
      totalCrias,
      criasHembra: partos.reduce((sum: number, m: any) => sum + (m.criasHembra || 0), 0),
      criasMacho: partos.reduce((sum: number, m: any) => sum + (m.criasMacho || 0), 0),
      tasaExito: totalMontas > 0 ? ((partos.length / totalMontas) * 100).toFixed(1) : 0,
      promedioXParto: partos.length > 0 ? (totalCrias / partos.length).toFixed(1) : 0,
      topMachos,
    };
